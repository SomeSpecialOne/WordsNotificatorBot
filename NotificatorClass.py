...
class User:
    ...
    def time_check(self) -> bool:
        ...
        """
        current_hour = time.gmtime(time.time() + (3600 * self.time_zone)).tm_hour
        if self.sleep_from > self.sleep_to:
            if current_hour in range(self.sleep_to, self.sleep_from + 1):
                return True
            else:
                return False
        else:
            if current_hour not in range(self.sleep_from + 1, self.sleep_to):
                return True
            else:
                return False

    def sleep_time_check(self):
        """
        Checking if current sending is last for this day to send good night sticker
        """
        if self.period == 3600:
            return False
        elif self.period == 7200:
            current_hour = time.gmtime(time.time() + (3600 * self.time_zone)).tm_hour + 2
        else:
            current_hour = time.gmtime(time.time() + (3600 * self.time_zone)).tm_hour + 3
        if self.sleep_from > self.sleep_to:
            if current_hour in range(self.sleep_to, self.sleep_from + 1):
                return False
            else:
    ...
    def logging(self, text: Exception):
        with open('logs.txt', 'a') as logs:
            logs.write(
                f'[{time.strftime("%H:%M", time.gmtime(time.time() + (3600 * 3)))}] {text} for {self.first_name} [{self.sleep_from}-{self.sleep_to}, {self.period}]\n')
...
class Notificator:
    ...
    async def start(self):
        ...
        for user in self.users:
            ...
            if user.period == 3600:
                if user.time_check() is True:
                    if current_hour == user.sleep_from or user.sleep_time_check():
                        try:
